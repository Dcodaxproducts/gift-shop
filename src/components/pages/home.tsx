      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_0.48fr]">
            ...
            <div className="flex h-[190px] items-end gap-2 sm:gap-3">
              {monthlyRevenueTrends.map((item) => (
                <div key={item.month} className="flex flex-1 flex-col items-center gap-3">
                  <div className="relative flex h-[150px] w-full items-end overflow-hidden rounded-t-sm bg-primary/10">
                    <div
        ...
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="p-5">
            <div className="mx-auto my-5 flex size-44 items-center justify-center rounded-full bg-[conic-gradient(var(--primary)_0_65%,#e2e8f0_65%_100%)]">
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="p-0">
            <div className="grid grid-cols-[1.2fr_1fr_0.8fr] border-b border-border px-5 pb-3 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
              <span>Provider</span>
            ...
            <div className="divide-y divide-border">
              {providerPerformance.map((provider) => (
                <div
                  ...
                  className="grid grid-cols-[1.2fr_1fr_0.8fr] items-center gap-4 px-5 py-4"
                >
                  <div className="flex items-center gap-3">
                    <span className="flex size-8 items-center justify-center rounded-lg bg-primary/10 text-[10px] font-semibold text-primary">
                      {provider.shortCode}
                    </span>
                    <span className="max-w-[92px] text-xs font-semibold leading-4 text-slate-700">
                      {provider.provider}
                    </span>
                  </div>
        ...
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="p-0">
            <div className="grid grid-cols-[0.9fr_1fr_0.8fr] border-b border-border px-5 pb-3 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
              <span>ID & User</span>
              <span>Reason</span>
              <span>Status</span>
            </div>
            <div className="divide-y divide-border">
              {recentDisputes.map((dispute) => (
                <div
                  key={dispute.id}
                  className="grid grid-cols-[0.9fr_1fr_0.8fr] items-center gap-4 px-5 py-4"
                >
                  <div>
                  ...
                  <span
                    className={cn(
                      "w-fit rounded-full px-3 py-2 text-[10px] font-semibold",
